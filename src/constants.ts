...
// Companion config
export const DEFAULT_MIDI_TCP_PORT = 51325
...
export const EQ_FREQUENCY_CHOICES: { label: string; id: number }[] = times((n: number) => {
	const frequency = midiValueToEqFrequency(n)
	const label = frequency < 1000 ? `${frequency} Hz` : `${(frequency / 1000).toFixed(2)} kHz`
	return { label, id: n }
})(128)

export const HPF_FREQUENCY_CHOICES: { label: string; id: number }[] = times((n: number) => {
